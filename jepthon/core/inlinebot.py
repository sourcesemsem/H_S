...
def main_menu():
    text = f"**- [سـورس سـيـمـو](https://t.me/FTTUTY)\
        \n المسـاعد\
        \n\
        \n◽ الجوكر لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" semo UsᴇʀBoᴛ", "https://t.me/FTTUTY"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس سـيـمـو🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس سيمو 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس سيمو 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت سيمو 🧸♥](https://t.me/FTTUTY)**\
            \n\
            \n❤ سيمو هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [جروب المسـاعدة](https://t.me/FTTUTT3)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="jepiq 🧸♥",
            description="ادخـل جـروب المسـاعدة",
            url="https://t.me/FTTUTT3",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
