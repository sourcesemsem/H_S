...
@jepiq.on(admin_cmd(pattern="سيمو(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"سماحة سيمو مطور سورس سيمو @FTTUTY - @DEV_SAMIR")
...
async def OwnerStart(event):
    ...
    if sender.id == ownersayed1_id :
        order = await event.reply('اهلا بـڪ مطوري سمير - @DEV_SAMIR')
